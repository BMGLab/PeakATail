...
\subsection*{The question this experiment answers}

The framing that matters is \textbf{not} ``how many PAS did the filter drop'' but
\textbf{``did dropping them change a conclusion''}. A filter that removes 40\% of
PAS while moving cell-type agreement by 0.01 and flipping no trend signs is
cosmetically large and scientifically cheap; a filter that flips trend signs
retroactively invalidates every atlas-snapped result in the project's history.
Section~\ref{sec:q_atlas} has already established that the filters cannot be
judged on atlas agreement, so the downstream readout is the only available test.

\subsection*{Analysis skeleton (drops in when the run completes)}

The analysis is implemented and wired --- \texttt{cross\_experiment.filter\_effect\_deltas()}
emits all five tables below against the \texttt{baseline} scenario --- so this
section is populated by re-running the generator, with no new code. Every row is
a difference from \texttt{baseline}, which by construction keeps every PAS.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{lXl}
    \toprule
    \textbf{Pipeline step} & \textbf{Metric reported per scenario} & \textbf{Source table} \\
    \midrule
    PAS catalogue & PAS kept, $\Delta$ vs baseline, fraction retained &
      \texttt{filter\_effect\_pas.tsv} \\
    Feature space & PAS kept in results vs PAS used for clustering &
      \texttt{filter\_effect\_pas.tsv} \\
    Clustering & $n$ clusters, ARI/AMI vs baseline clustering &
      \texttt{filter\_effect\_clustering.tsv} \\
    Cell typing & \textbf{\% of cells whose cell-type call changes} &
      \texttt{filter\_effect\_celltype\_shift.tsv} \\
    Differential APA & significant PAS/genes; hit-set overlap with baseline &
      \texttt{filter\_effect\_fisher.tsv} \\
    3\textquotesingle{}UTR trend & per-cell-type slope; \textbf{sign flips} vs baseline &
      \texttt{filter\_effect\_trend.tsv} \\
    \bottomrule
  \end{tabularx}
  \caption{The blast radius of one filter, one row per downstream step. Figure
    slots 20--23 are reserved for the corresponding panels.}
  \label{tab:filter_effect_skeleton}
\end{table}

\textbf{Status at the time of writing.} The scenario directories present on disk
are from an aborted earlier launch (restricted to a single GSM rather than the
intended 6-GSM subset) and have been set aside as
\texttt{\_stale\_*}; no \texttt{B2\_gex\_celltyping} or \texttt{B3\_switch}
outputs exist for any scenario. \textbf{Nothing in this section may be cited
until all four scenarios complete on the 6-GSM subset.}
...
data. \textbf{Slots 20--23 are reserved} for the pending filter-effect panels
(Section~\ref{sec:filter_effect}).
