...
\noindent \textbf{The report is organised around the experiments, not the
pipeline.} Section~\ref{sec:experiments} is the spine: each subsection states the
question an experiment was run to answer, shows the view that discriminates
between its arms, and gives the verdict. The remaining sections document the
methods behind those numbers and the per-run detail.

\medskip
\noindent \textbf{Headline verdicts.} One result is positive and transferable:
\emph{TF-IDF/LSI normalisation is the pipeline's most consequential design
choice}, beating library-size normalisation by $\Delta$ARI $= 0.140$ against
independent expression-derived cell-type labels, with every other swept parameter
at least three times smaller (Section~\ref{sec:q_knobs}). Four are negative and
overturn earlier claims: the benchmark's F1 ranking of peak callers is a
peak-\emph{count} ranking and its precision is $\approx$70\% attributable to
chance against a null (Sections~\ref{sec:validation},~\ref{sec:q_atlas}); the
choice of peak caller does not reach the biology --- two strategies sharing only
29\% of their PAS differ by 0.001 in cell-type agreement
(Section~\ref{sec:q_strategy}); the cohort-wide ``global
3\textquotesingle{}UTR shortening'' is a detection-rate artifact, with per-stage
mean PDUI tracking per-stage sequencing depth at median $r = 0.996$
(Sections~\ref{sec:q_shortening},~\ref{sec:corrected}); and the differential-APA
hit lists are pseudoreplicated over reads, calling a median 43\% of tested PAS
significant (Section~\ref{sec:q_fisher}). A reproducibility defect that
invalidates the trim axis's PAS-yield numbers is documented in
Section~\ref{sec:q_trim}, and the filter-effect experiment
(Section~\ref{sec:filter_effect}) is still running.
\end{abstract}
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Experiment-Driven Findings}
\label{sec:experiments}
% ─────────────────────────────────────────────────────────────────────────────

This section is the spine of the report. Each subsection states the
\textbf{question} an experiment was run to answer, shows the \textbf{view that
discriminates} between its arms, and gives a \textbf{verdict} --- including where
the verdict is ``no effect'' or ``this metric cannot tell''. The methods that
produced each number are documented in Sections~\ref{sec:peak_calling}--%
\ref{sec:length}; the detailed tables are in Section~\ref{sec:corrected}.

Two analyses were run independently over the same sweep and are reported
together throughout: a per-run harvest
(\texttt{ema.benchmark.sweep\_analysis}, Figures 01--19) and a cross-run
comparison (\texttt{ema.benchmark.cross\_experiment}, Figures 24--36). Where they
agree, the agreement is itself evidence; where only one can answer a question,
that is stated.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{llX}
    \toprule
    \textbf{Phase} & \textbf{Axis} & \textbf{Arms} \\
    \midrule
    1 & Cohort headline & 17 GSM, \texttt{lambda\_gradient}, atlas + ip in \emph{annotate} mode \\
    2 & Peak strategy & \texttt{lambda\_gradient} / \texttt{lambda\_poisson} / \texttt{sierra\_iterative} \\
    2 & Internal priming & \texttt{annotate} / \texttt{filter} / \texttt{off} \\
    2 & Trim & \texttt{max\_gene\_distance} 1--10\,kb; \texttt{utr\_multiplier} 1.5--3.0; \texttt{include\_extended} \\
    2 & Clustering & resolution 0.5/1.0/2.0; \texttt{n\_neighbors} 15/30/50; \texttt{leiden\_tfidf} vs \texttt{leiden\_libsize} \\
    3 & Filter effect & baseline / atlas / annot-3\textquotesingle{}UTR / ip \hfill \textbf{(pending, Section~\ref{sec:filter_effect})} \\
    1 & Switch biology & 24 cell types $\times$ 4 stages (Normal $\rightarrow$ StageI $\rightarrow$ IVprimary $\rightarrow$ Met) \\
    \bottomrule
  \end{tabularx}
  \caption{The experiments behind this report. Two structural constraints govern
    every comparison: the trim and clustering axes branch off the \emph{same}
    saved peak-calls (so they share a PAS catalogue and differ only downstream),
    while the strategy axis re-calls peaks; and Phase 2--3 run on a 6-GSM subset
    against Phase 1's full 17-GSM cohort, so cross-phase numbers are confounded
    by cohort size.}
  \label{tab:experiment_axes}
\end{table}

% -----------------------------------------------------------------------------
\subsection{Does the peak-calling strategy change the answer?}
\label{sec:q_strategy}

\textbf{Question.} Does the choice of peak caller change \emph{which} PAS are
found, \emph{how good} they are, and does any of that reach the biology?

\textbf{Discriminating view.} PAS-set agreement between arms
(Figure~\ref{fig:pas_jaccard}), set against cell-type recovery
(Figure~\ref{fig:yield_vs_ari}).

\begin{figure}[htbp]
  \centering
  \begin{subfigure}{0.48\textwidth}
    \includegraphics[width=\textwidth]{figures/corrected/26_pas_set_jaccard.png}
    \caption{}\label{fig:pas_jaccard}
  \end{subfigure}\hfill
  \begin{subfigure}{0.50\textwidth}
    \includegraphics[width=\textwidth]{figures/corrected/27_yield_vs_celltype_ari.png}
    \caption{}\label{fig:yield_vs_ari}
  \end{subfigure}
  \caption{(a) Pairwise PAS-set agreement (Jaccard within $\pm$50\,bp).
    (b) PAS yield against agreement with GEX cell types; note the compressed
    $y$-axis --- the line is flat.}
  \label{fig:strategy_crossexp}
\end{figure}

\textbf{Verdict: strategy choice is a decision about the PAS catalogue, not
about the biology.} \texttt{lambda\_gradient} and \texttt{lambda\_poisson} agree
on only \textbf{29\%} of their sites (Jaccard 0.294; each recovers $\approx$44\%
of the other) --- yet their agreement with independent GEX cell types differs by
\textbf{0.0003 ARI} (0.4002 vs 0.4005). \texttt{sierra\_iterative} calls
1.90$\times$ more PAS and is close to a \emph{superset}: it recovers 99.2\% of
\texttt{lambda\_gradient}'s calls (Jaccard 0.52), matching the containment
asymmetry in Figure~\ref{fig:count_controlled}c. Cell-type recovery across all
five arms spans 0.4002--0.4116.

\textbf{Three independent lines converge on this verdict}, and it is worth
stating once that they are independent rather than repeating the claim in each
section. (i) The call sets are \emph{nested}: count-controlled to a common $n$,
the arms are indistinguishable at a 0.15\,pp spread and
\texttt{lambda\_gradient} is fully contained in \texttt{sierra\_iterative}
(Figure~\ref{fig:count_controlled}), which the symmetric Jaccard above
reproduces from the other direction. (ii) The downstream statistic does not
move: $\Delta$ARI $= 0.011$ across all five arms. (iii) The cluster
$\rightarrow$ cell-type flow survives the strategy swap qualitatively
(Figure~\ref{fig:sankey_strategies}). A set-overlap argument, an external
agreement metric and a visual mapping are three different ways to be wrong;
agreeing, they are strong evidence that \textbf{the strategy axis is inert
downstream}.

Two controls confirm the axis behaved as designed: \texttt{lg\_ip\_off} is
\textbf{byte-identical} to \texttt{lg\_annotate} (Jaccard 1.000), so
\texttt{-{}-ip-filter-mode annotate} is a verified no-op on the PAS set; and
\texttt{lg\_ip\_filter} is a strict subset removing 8.94\% of peaks.

\textbf{Practical consequence.} Choose the caller by what you want the catalogue
to be --- \texttt{sierra\_iterative} for completeness (2$\times$ the sites, most
multi-PAS genes), \texttt{lambda\_poisson} for the tightest peaks (403\,bp median
vs 640\,bp) --- not by a benchmark score, and not expecting it to move the result.

% -----------------------------------------------------------------------------
\subsection{Is ``precision \texorpdfstring{$\approx$}{~} 1.0'' evidence of accuracy?}
\label{sec:q_atlas}

\textbf{Question.} Every arm scores precision 0.996--0.999 against PolyASite v3.0.
Is that a measurement?

\textbf{Discriminating view.} A null control. Each called PAS is displaced by
$\pm U(5\,\text{kb}, 50\,\text{kb})$ on its own chromosome --- staying in the same
genomic neighbourhood --- and precision recomputed
(Figure~\ref{fig:atlas_null}). This is the quantitative complement to the
saturation shown in Figure~\ref{fig:atlas_saturation}: that figure shows
precision \emph{cannot vary}; this one shows \emph{how much of it is free}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/corrected/24_atlas_null_control.png}
  \caption{Observed precision against two nulls, per arm and cutoff. At
    $\pm$50\,bp a randomly displaced PAS already scores \textbf{0.702}.}
  \label{fig:atlas_null}
\end{figure}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{rrrrr}
    \toprule
    cutoff & observed & null (shifted) & null (uniform) & \textbf{excess} \\
    \midrule
    10\,bp   & 0.472 & 0.261 & 0.130 & \textbf{+0.211} \\
    25\,bp   & 0.822 & 0.514 & 0.271 & \textbf{+0.308} \\
    50\,bp   & 0.947 & 0.702 & 0.409 & \textbf{+0.245} \\
    100\,bp  & 0.986 & 0.842 & 0.550 & +0.144 \\
    200\,bp  & 0.995 & 0.923 & 0.677 & +0.072 \\
    \bottomrule
  \end{tabular}
  \caption{\texttt{lg\_annotate} against PolyASite v3.0. With 18,432,135
    single-base reference entries --- one every $\approx$168\,bp of genome --- a
    $\pm$50\,bp window is atlas-positive for much of the genome by construction.}
  \label{tab:atlas_null}
\end{table}

\textbf{Verdict: roughly 70\% of the headline precision is free, and the metric
cannot rank strategies.} Excess-over-null at 50\,bp is +0.245, +0.243, +0.221 and
+0.249 for \texttt{lg}, \texttt{lp}, \texttt{si} and \texttt{lg\_ip\_filter} --- a
spread of \textbf{0.028} across arms that disagree on more than half their PAS
(Section~\ref{sec:q_strategy}). Figure~\ref{fig:atlas_excess} shows the arms
overlapping at every cutoff.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.72\textwidth]{figures/corrected/25_atlas_excess_over_null.png}
  \caption{Excess precision over the shifted null. The arms are
    indistinguishable at every cutoff.}
  \label{fig:atlas_excess}
\end{figure}

\textbf{There is real signal, and it should be reported at the cutoff where it
lives:} called PAS sit a median \textbf{11\,bp} from the nearest atlas entry
versus \textbf{24\,bp} for the displaced null, and at $\pm$10\,bp the caller beats
chance by 0.211. Report precision at $\pm$10--25\,bp \emph{with} the null; never
$\pm$50\,bp bare. Together with the yield-ranking result
(Section~\ref{sec:validation}) and the count-controlled collapse
(Figure~\ref{fig:count_controlled}), this retires the atlas-based strategy
ranking on three independent grounds.

% -----------------------------------------------------------------------------
\subsection{Does the annotation window matter?}
\label{sec:q_trim}

\textbf{Question.} How far past an annotated gene end may a PAS be assigned, and
does widening that window change anything?

\textbf{Discriminating view.} Cell-type agreement and cluster count against
\texttt{max\_gene\_distance}, scaled against a knob that \emph{does} matter
(Figure~\ref{fig:trim_sensitivity}).

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/corrected/28_trim_sensitivity.png}
  \caption{Trim window against cell-type agreement, cluster count and PAS yield.
    The shaded band on the left panel is the range spanned by the
    \texttt{tfidf}$\rightarrow$\texttt{libsize} change, for scale.}
  \label{fig:trim_sensitivity}
\end{figure}

\textbf{Verdict: inert over the range swept.} A 10$\times$ change in
\texttt{max\_gene\_distance} moves cell-type ARI by \textbf{0.007} and the mean
cluster count by 0.24 (Figure~\ref{fig:ip_and_trim}c). \texttt{utr\_multiplier}
moves ARI by 0.012. All \texttt{\_ext} branches sit at ARI-vs-reference 0.78--0.80
\emph{regardless of distance}, which locates the effect precisely:
\textbf{\texttt{include\_extended} is the real knob on this axis;
\texttt{max\_gene\_distance} is not.} Two independent analyses reach this
conclusion from different statistics, which is the strongest form the result can
take.

\begin{quote}
\textbf{\textcolor{red}{Reproducibility defect --- PAS-yield numbers on this axis
are withdrawn.}} Five branches declaring \emph{identical} trim parameters
(\texttt{\{5000, 2.0, false\}}) produced \textbf{three different}
\texttt{pas\_gene.tsv} files (Figure~\ref{fig:reannotate_repro}). The PAS
catalogue is identical across all of them (\texttt{annotatedpas.bed} = 184,295
rows everywhere, no duplicate rows), so \emph{only the PAS$\rightarrow$gene
assignment diverges}; and clustering parameters cannot affect gene assignment.
The md5 groups track \textbf{write time}, the signature of a shared-state race
between concurrently executing \texttt{ema reannotate} branches.

The magnitude is disqualifying: the parameter-free spread
(51,129 $\leftrightarrow$ 67,409 assignments) \textbf{exceeds} the spread the trim
sweep is trying to measure (58,982 for \texttt{d5000\_ext} vs 67,409 for
\texttt{default}). \textbf{The noise is larger than the signal.} PAS-yield
comparisons on the trim axis must not be cited until the axis is re-run
serially. The per-cell clustering ARIs above are far less exposed --- the
divergence is in feature assignment, not cell assignment --- but should be
re-confirmed on that re-run.
\end{quote}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/corrected/30_reannotate_reproducibility.png}
  \caption{Identical declared trim parameters; three distinct PAS$\rightarrow$gene
    assignment tables. Colour marks the md5 group; groups follow write time, not
    parameters.}
  \label{fig:reannotate_repro}
\end{figure}

% -----------------------------------------------------------------------------
\subsection{Which parameter actually controls the result?}
\label{sec:q_knobs}

\textbf{Question.} Of everything the pipeline exposes, what actually changes the
biological readout?

\textbf{Discriminating view.} Every swept knob on one axis, measured against an
\emph{external} criterion --- agreement with independent GEX cell-type labels, not
an internal cluster-compactness score (Figure~\ref{fig:knob_ranking}).

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{figures/corrected/31_knob_ranking.png}
  \caption{Each swept parameter ranked by the full range (max $-$ min, default
    included) it spans in ARI against GEX cell types.}
  \label{fig:knob_ranking}
\end{figure}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrrrr}
    ...
    \midrule
    \texttt{BASE} / \texttt{A2\_trim\_default} & 14.8 & 1.000 & \textbf{0.4631} & \textbf{0.6259} & 0.122 \\
    \texttt{A3\_nn50}          & 13.8 & 0.885 & 0.4568 & 0.6216 & 0.126 \\
    \texttt{A2\_trim\_mult3.0} & 14.8 & 0.857 & 0.4533 & 0.6253 & 0.123 \\
    \texttt{A2\_trim\_d1000\_ext}  & 14.9 & 0.804 & 0.4510 & 0.6266 & 0.124 \\
    \texttt{A2\_trim\_d10000\_ext} & 14.9 & 0.783 & 0.4487 & 0.6279 & 0.127 \\
    \texttt{A3\_nn15}    & 16.5 & 0.822 & 0.4458 & 0.6224 & 0.116 \\
    \texttt{A3\_res0.5}  & 10.5 & 0.729 & 0.4283 & 0.6159 & \textbf{0.134} \\
    \texttt{A3\_res2.0}  & 22.4 & 0.642 & 0.4154 & 0.6078 & 0.101 \\
    \texttt{A3\_libsize} & 16.8 & \textbf{0.359} & \textbf{0.3234} & \textbf{0.4822} & n/a \\
    \bottomrule
  \end{tabular}
  \caption{Clustering branches, averaged over 17 datasets.
    \texttt{leiden\_libsize} produces no LSI embedding, so its silhouette is
    undefined rather than zero.}
  \label{tab:knob_branches}
\end{table}

\textbf{Verdict (a): TF-IDF/LSI normalisation is the pipeline's most consequential
design choice.} Replacing it with library-size normalisation costs
$\Delta$ARI $= \mathbf{0.140}$ (0.463 $\rightarrow$ 0.323) and
$\Delta$AMI $= 0.144$ against independent cell-type labels, and yields the most
fragmented partition. No other knob comes within a factor of three. To our
knowledge no published APA tool has benchmarked PAS-based clustering against
cell-type labels with ARI/AMI, which makes this the sweep's most transferable
result. Figure~\ref{fig:umap_variants} shows the same fact qualitatively: the
embedding is fixed and only the cut changes.

\textbf{Verdict (b): resolution has a genuine optimum at the default 1.0, and
silhouette is the wrong objective.} Cell-type agreement falls at both 0.5
(under-clustered) and 2.0 (over-clustered), while silhouette rises monotonically
as resolution falls (Figure~\ref{fig:resolution_curve}). An internal
compactness score will always prefer fewer, rounder clusters; it cannot see that
the clusters have stopped corresponding to cell identity. The same caution applies
to Sankey purity, which rises mechanically as clusters shrink
(Figure~\ref{fig:sankey_variants}).

\begin{figure}[htbp]
  \centering
  \begin{subfigure}{0.48\textwidth}
    \includegraphics[width=\textwidth]{figures/corrected/32_resolution_ari_silhouette.png}
    \caption{}\label{fig:resolution_curve}
  \end{subfigure}\hfill
  \begin{subfigure}{0.50\textwidth}
    \includegraphics[width=\textwidth]{figures/corrected/29_celltype_reassignment.png}
    \caption{}\label{fig:celltype_reassignment}
  \end{subfigure}
  \caption{(a) ARI against cell type peaks at resolution 1.0 while silhouette
    prefers ever-coarser clusters. (b) The blast radius of each parameter,
    expressed as the percentage of cells whose cell-type call changes.}
  \label{fig:clustering_knobs}
\end{figure}

\textbf{Verdict (c): expressed as consequences for a biologist}
(Figure~\ref{fig:celltype_reassignment}), \texttt{leiden\_libsize} changes the
cell-type call for \textbf{30.8\%} of cells, resolution 2.0 for 19.3\% and 0.5 for
14.4\%, while every trim branch sits at 6--10\% and \texttt{n\_neighbors}~50 at
4.0\%.

\begin{quote}
\textbf{Caveat.} \texttt{A3\_libsize} is one of the branches affected by the
assignment race in Section~\ref{sec:q_trim}. It received \emph{more} features than
the branches it loses to (82,702 assignments vs 51,129), so the direction of the
effect is unlikely to be explained by the defect --- extra features should help,
not hurt --- but the magnitude should be re-confirmed on a serial re-run.
\end{quote}

% -----------------------------------------------------------------------------
\subsection{Do 3\textquotesingle{}UTRs shorten as the tumour progresses?}
\label{sec:q_shortening}

\textbf{Question.} The sweep's central biological claim: within each cell type,
does mean PDUI fall along Normal $\rightarrow$ StageI $\rightarrow$ IVprimary
$\rightarrow$ Met?

\textbf{Discriminating view.} Three checks, each of which the claim must survive:
enough stages to fit a trend, a slope that is not the detection rate, and
significance.

\paragraph{Check 1 --- most trends are fit on too few stages.}
Only \textbf{18 of 24} cell types have $\geq$3 stages; six (25\%) have
\emph{two}, where Spearman $\rho$ is $\pm 1$ \textbf{by construction}
(Figure~\ref{fig:trend_confound}b). The shipped \texttt{length\_trend.json} for
\texttt{MESENCHYAL} reports $\rho = -1.0$ on \texttt{n\_stages} = 2. With three
stages $\rho$ can take only four values ($\pm$0.5, $\pm$1). Cell types below three
stages are greyed out in Figure~\ref{fig:pdui_heatmap} and must not be cited.

\paragraph{Check 2 --- mean PDUI \emph{is} sequencing depth.}
PDUI is 0 whenever a gene's distal PAS has no reads in a cell, and in this data
\textbf{95--99\% of gene--cell pairs have no reads at all}. Mean PDUI is therefore
arithmetically $(1 - \text{frac uninformative}) \times$ mean informative PDUI, and
the informative component is nearly flat:
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llrrrr}
    \toprule
    cell type & stage & mean PDUI & PDUI (informative) & frac uninform. & mean reads \\
    \midrule
    ...
    \bottomrule
  \end{tabular}
  \caption{The apparent stage trend in column 3 is the depth trend in column 6.
    Across cell types the correlation between per-stage mean PDUI and per-stage
    mean reads is \textbf{median $r = 0.9957$} --- 15 of 18 interpretable cell
    types above 0.95, 11 above 0.99.}
  \label{tab:pdui_depth}
\end{table}

This reproduces, from a different statistic, the $r = -0.997$ between reported
PDUI and zero-coverage fraction in Figure~\ref{fig:depth_confound}.

\paragraph{Check 3 --- the direction is not robust and nothing is significant.}
Refitting on informative pairs only flips the sign in \textbf{9 of 24} cell types
(37.5\%) and drops the ``decreasing'' fraction from 0.833 to 0.708
(Figure~\ref{fig:trend_confound}a). Median slope is $-0.0021$
($-0.0033$ controlled) --- 0.2--0.3\% of the 0--1 PDUI scale per stage. Across the
18 interpretable cell types exactly \textbf{one} reaches nominal $p < 0.05$
(\texttt{SEROUS\_CELLS}, $p = 0.044$), which is what 18 tests at $\alpha = 0.05$
produce by chance and survives no correction.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/corrected/34_trend_depth_confound.png}
  \caption{(a) Slope fitted on all gene--cell pairs against slope fitted on
    informative pairs only; points off the diagonal are confounded, points in the
    off-sign quadrants flip direction. (b) Stages available per cell type.}
  \label{fig:trend_confound}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.62\textwidth]{figures/corrected/33_pdui_celltype_stage.png}
  \caption{Mean PDUI by cell type $\times$ stage. Cell types with fewer than
    three stages are flagged: their trend statistics are undefined.}
  \label{fig:pdui_heatmap}
\end{figure}

\textbf{Verdict: there is no defensible evidence of stage-dependent
3\textquotesingle{}UTR shortening in this cohort.} The apparent trend is a
detection-rate artefact of the classic PDUI estimator under 95--99\% dropout.
Asking the question properly requires either an estimator that conditions on
detection (report informative-only PDUI with per-stage depth as a covariate) or
depth-matched subsampling across stages --- neither of which this sweep ran.
\textbf{This supersedes the ``global 3\textquotesingle{}UTR shortening'' claim.}
Consistent with Section~\ref{sec:length}, only \texttt{classic} and
\texttt{shannon} are used here; \texttt{proportion} is invalid (98.2\% uniform
$1/n_{\text{PAS}}$ padding) and no length metric rescues the stage signal.

% -----------------------------------------------------------------------------
\subsection{Are the differential-APA hits trustworthy?}
\label{sec:q_fisher}

\textbf{Question.} \texttt{ema switch diff} returns thousands of significant PAS
per cell type. Can they be read as biology?

\textbf{Discriminating view.} Hit counts against cell counts \emph{and} effect
sizes (Figure~\ref{fig:fisher_pseudo}).

\textbf{Verdict: no.} Across 90 cell-type $\times$ contrast tests, Fisher declares
a \textbf{median 43\%} of tested PAS significant at FDR 0.05, reaching
\textbf{73.7\%}. Significance tracks cell count
(Spearman $\rho = 0.556$, $p = 1.3\times10^{-8}$) because the test is applied to
\emph{read} counts pooled over cells, so each cell contributes many correlated
observations. The relationship to effect size is \textbf{inverse}:
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llrrrrr}
    \toprule
    cell type & contrast & cells & tests & sig. & frac & median $|\Delta p|$ \\
    \midrule
    \texttt{TREG}            & Normal vs StageI & 3,309 & 18,795 & 13,842 & \textbf{0.737} & \textbf{0.189} \\
    \texttt{MONOCYTIC}       & Normal vs StageI & 2,293 & 19,084 & 13,744 & 0.720 & 0.179 \\
    \texttt{EXHAUSTED\_REGUL}& Normal vs StageI &   930 & 23,599 & 15,096 & 0.640 & 0.207 \\
    \midrule
    \texttt{SEROUS\_CELLS}   & IVprimary vs Met &   220 &  4,815 &  1,284 & 0.267 & \textbf{0.781} \\
    \texttt{MACROPHAGE\_M1}  & IVprimary vs Met &   175 &  4,454 &    773 & 0.174 & \textbf{0.820} \\
    \bottomrule
  \end{tabular}
  \caption{High-power contrasts return $\approx$14,000 hits at a median effect of
    0.19; low-power contrasts return $\approx$800 hits at a median effect of 0.82.
    Every one of the ten largest hit lists is \texttt{Normal\_vs\_StageI} --- the
    contrast with the most cells.}
  \label{tab:fisher_power_crossexp}
\end{table}

\begin{figure}[htbp]
  \centering
  \begin{subfigure}{0.48\textwidth}
    \includegraphics[width=\textwidth]{figures/corrected/35_fisher_pseudoreplication.png}
    \caption{}\label{fig:fisher_pseudo}
  \end{subfigure}\hfill
  \begin{subfigure}{0.50\textwidth}
    \includegraphics[width=\textwidth]{figures/corrected/36_recurrent_switches.png}
    \caption{}\label{fig:recurrent_crossexp}
  \end{subfigure}
  \caption{(a) Significant PAS against cells in the contrast, sized by the median
    effect of the hits. (b) Recurrence across cell types, and the most recurrent
    genes.}
  \label{fig:switch_trust}
\end{figure}

\paragraph{Recurrence does not rescue the hit lists --- it condemns them.}
The analysis plan proposed filtering hits by recurrence across cell types, on the
...
that filter fails in the informative direction: of \textbf{7,941} genes with a
significant switch, only \textbf{18.6\%} are private to one cell type, 72\% recur
in $\geq$3 and 58\% in $\geq$5. Each of the 15 most recurrent genes is significant
in \textbf{all 24 of 24} cell types at the maximum possible effect size, with
\emph{inconsistent direction} --- called proximal in some cell types and distal in
others. Real cell-type-specific APA does not look like this. The complementary
per-gene analysis reaches the same place from the other side: private genes carry
\emph{larger} mean $|$slope$|$ than recurrent ones (Figure~\ref{fig:recurrence}),
so recurrence indexes testability, not shared regulation.

\textbf{Consequence: no gene list from the current \texttt{fisher} output should
be cited.} Use it as a within-gene screen ranked by effect size with a
$|\Delta p|$ floor, or move to a test with cells as the replication unit ---
\texttt{nb\_pairwise}, which \textbf{was not run} in this sweep.
\texttt{nb\_multi} is not the alternative: it calls 99.8\% of PAS significant
(Section~\ref{sec:switch_test}).

% -----------------------------------------------------------------------------
\subsection{Claims the data does not support}
\label{sec:q_corrections}

Collected for the reader who knows the earlier drafts. Each row is corrected in
the section cited.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{XXl}
    \toprule
    \textbf{Earlier claim} & \textbf{What the data shows} & \textbf{See} \\
    \midrule
    Precision $\approx$ 1.0 against PolyASite v3.0 demonstrates accuracy &
    $\approx$70\% of it is free; excess over null $= +0.245$ at $\pm$50\,bp &
    \ref{sec:q_atlas} \\
    \texttt{sierra} $>$ \texttt{lambda\_gradient} by F1 &
    F1 ranks yield; count-controlled the arms are nested and indistinguishable &
    \ref{sec:validation}, \ref{sec:q_atlas} \\
    Peak strategy is a meaningful methodological choice for the biology &
    29\% PAS overlap, $\Delta$ARI 0.011 --- it is a catalogue choice &
    \ref{sec:q_strategy} \\
    Global 3\textquotesingle{}UTR shortening across stages &
    mean PDUI $\equiv$ depth ($r = 0.996$); 37.5\% sign flips; nothing significant &
    \ref{sec:q_shortening} \\
    Per-cell-type Spearman $\rho$ from \texttt{length\_trend.json} &
    $\rho = \pm1$ by construction for the six two-stage cell types &
    \ref{sec:q_shortening} \\
    ``$N$ genes switch'' from \texttt{fisher} &
    median 43\% of all tested PAS are called significant &
    \ref{sec:q_fisher} \\
    Recurrent switch genes are the high-confidence set &
    81\% of genes recur; the most recurrent have inconsistent direction &
    \ref{sec:q_fisher} \\
    Trim-parameter PAS-yield differences &
    confounded by a concurrency race larger than the effect &
    \ref{sec:q_trim} \\
    \texttt{canonical\_cluster} from \texttt{switch match} &
    matching is saturated (confidence 0.0102 for every pair) --- unusable &
    \ref{sec:cross_dataset_match} \\
    \midrule
    \emph{(new, positive)} &
    TF-IDF beats library-size normalisation by $\Delta$ARI 0.140 &
    \ref{sec:q_knobs} \\
    \bottomrule
  \end{tabularx}
  \caption{Corrections and the one new positive result.}
...
\begin{quote}
\textbf{\textcolor{red}{PROVISIONAL --- pending the corrected differential
engine.}} \texttt{fisher} is being replaced by a per-cell test
(\texttt{mwu\_percell}) that fixes the pseudoreplication documented in
Section~\ref{sec:q_fisher}, and \texttt{proportion} by a corrected length
engine. Sections~\ref{sec:atlas_diff_strategies}, \ref{sec:atlas_volcano},
\ref{sec:atlas_top_genes} and~\ref{sec:atlas_enrichment} are computed
\emph{with} \texttt{fisher} and will be regenerated against the replacement
before the report is final.

\textbf{A distinction worth holding onto, because it decides what survives:}

\begin{itemize}[nosep]
  \item Findings \emph{about} \texttt{fisher}'s misbehaviour --- the 43\%
    significance rate, the cell-count coupling, the $|\Delta p| \approx 1$
    saturation, \texttt{nb\_multi}'s 99.7\%, and the below-chance agreement
    between them --- are \textbf{final}. They characterise the engine being
    replaced and are the evidence that motivated replacing it. A corrected test
    cannot retroactively make the old one well-behaved.
  \item Findings that \emph{use} \texttt{fisher} as a measuring instrument ---
    which genes switch, in which direction, how recurrent they are, and what
    pathways they enrich --- are \textbf{provisional} and must not be cited
    until regenerated.
\end{itemize}

Regeneration is one command per stage against the corrected run; see
\texttt{reports/REFRESH\_CHECKLIST.md}. Everything outside this section and
Section~\ref{sec:q_fisher} --- clustering, the atlas null control, the TF-IDF
result, the depth confound, \texttt{classic}/\texttt{shannon} --- is independent
of both engines and final.
\end{quote}
...
\textbf{Verdict: it does not escape, and it fails in a way that matters more
than the trend question.}

First the trend. \texttt{classic} PDUI falls steeply as reported
($0.0175 \rightarrow 0.0073$ across the four stages) and stops falling once
rows without reads are dropped --- the confound of
Section~\ref{sec:q_shortening}. \texttt{shannon} does \emph{not} reproduce that
pattern: it shows no stage trend in either form. As reported only
\textbf{2 of 18} interpretable cell types decrease (median slope
$+0.0021$); conditioned, \textbf{8 of 18} do (median $+0.0016$), with
\textbf{10 of 18 flipping sign} between the two. So the two metrics do not
corroborate a shortening signal --- one shows an artefact and the other shows
nothing.

Second, and more important: \textbf{\texttt{shannon} carries the same
padded-constant defect that invalidates \texttt{proportion}.} A gene--cell pair
with zero reads is assigned entropy exactly $1.0$ (uniform over its PAS) rather
than being left undefined, and \textbf{94--97\% of rows are exactly that
constant}. The consequence is visible in the means: reported entropy is
$\approx 1.05$ while entropy over informative rows is $\approx 1.79$ --- the
reported value is essentially the padding. Per-stage reported entropy tracks the
uninformative fraction at \textbf{median $r = 0.994$}, the same coupling
strength with which \texttt{classic} tracks depth.

\begin{quote}
\textbf{This extends the \texttt{proportion} finding rather than repeating it.}
Section~\ref{sec:length} marks \texttt{proportion} invalid at 98.2\% uniform
padding. The same audit applied to \texttt{shannon} gives 94--97\%. Two of the
three length metrics substitute a constant for missing data instead of
propagating missingness, and the third (\texttt{classic}) substitutes zero ---
which is why all three track coverage. \textbf{The defect is in how the length
layer handles absent observations, not in any one statistic}, so a fourth metric
would not fix it. The fix is to propagate missingness and report a
coverage-conditioned estimate with the covered-row count beside it.
\end{quote}

A slot is reserved here for \texttt{proportion} once its corrected engine output
is available, so the three metrics can be compared on equal terms; the
comparison to make at that point is whether the corrected engine propagates
missingness or merely changes the constant.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrr}
    \toprule
    Hallmark set (recurrent switch genes) & overlap & odds ratio & $q$ \\
    \midrule
    \texttt{MITOTIC\_SPINDLE}           & 116 & 2.04 & 0.0015 \\
    \texttt{UV\_RESPONSE\_DN}           &  83 & 2.11 & 0.0054 \\
    \texttt{PROTEIN\_SECRETION}         &  68 & 2.27 & 0.0054 \\
    \texttt{TNFA\_SIGNALING\_VIA\_NFKB} & 102 & 1.83 & 0.0063 \\
    \texttt{TGF\_BETA\_SIGNALING}       &  36 & 3.30 & 0.0063 \\
    \texttt{UNFOLDED\_PROTEIN\_RESPONSE}&  61 & 2.24 & 0.0063 \\
    \bottomrule
  \end{tabular}
  \caption{Top Hallmark sets for recurrent switch genes ($\geq$3 cell types;
    $n = 5{,}568$ of a 9,632-gene tested background). \textbf{9 of 50} sets reach
    FDR $\leq 0.05$. For cell-type-private switch genes ($n = 1{,}507$),
    \textbf{0 of 50} do.}
  \label{tab:hallmark_switch}
\end{table}

\textbf{Verdict: the enrichment is real as arithmetic and uninterpretable as
biology.} Three things block a pathway claim.

\begin{enumerate}[nosep]
  \item \textbf{The query is 58\% of the background.} 5,568 of 9,632 tested
    genes are "recurrent switch genes". A set that large is close to asking
    which Hallmark sets are enriched among well-covered genes.
  \item \textbf{The sets that pass are the ones detectability predicts} ---
    \texttt{MITOTIC\_SPINDLE}, \texttt{PROTEIN\_SECRETION},
    \texttt{UNFOLDED\_PROTEIN\_RESPONSE}: large, highly expressed, long-3\textquotesingle{}UTR
    gene families. That is the profile of genes that clear a coverage threshold,
    not of a polyadenylation programme.
  \item \textbf{The gene set itself is coverage-selected.}
    Section~\ref{sec:atlas_top_genes} showed every one of these calls has
    $|\Delta p| \approx 1$ and 82.9\% are called in both directions. Enrichment
    computed on that set inherits its selection.
\end{enumerate}

The contrast with the private set is the tell rather than a result: private
genes are simply less covered, so they are both harder to call repeatedly and
less likely to belong to abundant Hallmark families. This is the same pattern
Section~\ref{sec:corrected} found and correctly discounted --- COSMIC CGC crude
odds ratio 1.99 ($p = 6.7\times10^{-9}$) falling to \textbf{1.22 ($p = 0.28$)}
once detectability was controlled --- and the same one
Figure~\ref{fig:recurrence} shows from the effect-size side, where private genes
carry \emph{larger} mean $|$slope$|$ than recurrent ones.

\textbf{Reported for completeness and to document the correct background choice.
It should not be used to claim a biological programme.} A defensible version
needs the switch calls fixed first (Section~\ref{sec:atlas_top_genes}), then
enrichment against a background matched on expression and 3\textquotesingle{}UTR
length, not merely on being tested.
...
\subsection{Single-Cell Visualisation --- UMAP and PAS-Cluster $\rightarrow$ Cell-Type Flow}

All panels are built from real run artifacts on the analysis host:
\texttt{07\_clustering/<dataset>/clusters.h5ad} supplies \texttt{obsm['X\_umap']}
and \texttt{obs['leiden']}; \texttt{B2\_gex\_celltyping/<dataset>\_pas\_labeled.h5ad}
supplies \texttt{obs['celltype']}. Cell types are the 28 signature-scored labels,
cleaned to display names. Regenerate with
\texttt{python reports/generate\_clustering\_figures.py}.

\begin{quote}
\textbf{Read every panel with the labelling rate in view.} Cell-type calls exist
for \textbf{53.0\% of the 55,422 clustered cells} cohort-wide, and the per-dataset
rate ranges from \textbf{27\% to 87\%}. The unlabelled remainder is drawn in grey
rather than dropped: discarding it would make the PAS-cluster $\rightarrow$
cell-type mapping look far cleaner than it is.
\end{quote}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/corrected/15_umap_cohort_stages.png}
  \caption{Cohort UMAP, one dataset per stage. Top row coloured by PAS-Leiden
    cluster, bottom row by GEX cell type --- same cells, same embedding.}
  \label{fig:umap_stages}
\end{figure}

The PAS-space embedding is structured, not noise: cells form discrete islands and
the cell-type colouring lands coherently on them. That is the qualitative
counterpart to the ARI 0.463 / AMI 0.626 reported above --- real but partial
agreement.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/corrected/16_umap_clustering_grid.png}
  \caption{The six clustering variants on one shared dataset
    (GSM3516666-Normal). The embedding is essentially fixed; what changes is how
    finely it is cut.}
  \label{fig:umap_variants}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/corrected/17_sankey_clustering_grid.png}
  \caption{PAS cluster $\rightarrow$ GEX cell type per clustering variant, on the
    shared dataset. Left nodes are PAS clusters, right nodes cell types, ribbon
    width is cells.}
  \label{fig:sankey_variants}
\end{figure}

\textbf{Each panel is a single dataset, deliberately.} \texttt{leiden} ids are
assigned per dataset, so cluster ``0'' in one GSM is unrelated to cluster ``0'' in
another; pooling the 17 datasets merges unrelated clusters into one node and
produces a picture that cannot be read.

Most PAS clusters map predominantly to \emph{one} cell type: across all
experiments the median cluster sends \textbf{79\% of its labelled cells to a
...
predicts. Purity rises with resolution (0.75 at res 0.5, 0.81 at res 1.0, 0.84 at
res 2.0), but \textbf{this is not evidence that res 2.0 is better}: purity
increases mechanically as clusters shrink, and at the limit every singleton
cluster is perfectly pure.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/corrected/18_sankey_peak_strategies.png}
  \caption{The same flow per peak-calling strategy, on the shared grid dataset.
    The cluster $\rightarrow$ cell-type structure is preserved across
    lambda\_gradient, lambda\_poisson and sierra\_iterative, and under the
    internal-priming filter.}
  \label{fig:sankey_strategies}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/corrected/19_cluster_sizes.png}
  \caption{Cluster structure across all 19 experiments --- cluster count per
    dataset (a), size distribution across clustering variants (b), and
    fragmentation against cluster count (c).}
  \label{fig:cluster_sizes_corrected}
\end{figure}

Across every experiment the median dataset yields \textbf{15 clusters (range
7--29)}. Cluster-count differences track the resolution parameter, and the size
distributions are smooth rather than one dominant cluster plus fragments ---
consistent with the trim-insensitivity reported earlier.
...
\subsection{Strategy Comparison on the Corrected Cohort}

Both strategies run across all 24 cell types: \texttt{fisher} over the 6 stage
contrasts (90 cell-type $\times$ contrast tests), \texttt{nb\_multi} as one
omnibus per cell type. \textbf{\texttt{nb\_pairwise} was not run in this sweep},
so the three-way comparison of the superseded Figure~17 cannot be reproduced.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/corrected/12_diff_strategy.png}
  \caption{(a) significance rate per strategy; (b) hits against tests, with the
    diagonal marking ``every test significant''; (c) agreement between the two
    strategies.}
  \label{fig:diff_strategy}
\end{figure}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrrr}
    \toprule
    strategy & tests & PAS tested & PAS significant & rate \\
    \midrule
    \texttt{fisher}   & 90 (celltype $\times$ contrast) & 1,233,328 & 541,133 & \textbf{42.9\%} \\
    \texttt{nb\_multi} & 24 (celltype omnibus)          & 7,965     & 7,946   & \textbf{99.8\%} \\
    \bottomrule
  \end{tabular}
  \caption{Differential-APA strategies on the corrected cohort.}
  \label{tab:diff_strategy}
\end{table}

\textbf{Neither rate is credible, and they fail in different ways.}
\texttt{nb\_multi} calls essentially everything significant --- 99.8\% overall and
between 84.2\% and 100\% in every cell type, its points lying \emph{on} the
all-significant diagonal. An omnibus across four stages on hundreds of cells will
reject the null for any PAS with non-uniform usage, which is nearly all of them:
the test answers ``does this PAS vary at all'', not ``does it vary with stage''.
It is a variability screen, not a differential test. \texttt{fisher} calls 42.9\%,
for the pseudoreplication reason given later --- its contingency table is built
over reads, so effective $n$ is depth.

\textbf{They corroborate each other on about a quarter.} Only 24\% (median; mean
27.7\%) of \texttt{nb\_multi}'s significant PAS are also called by
\texttt{fisher}. Note the statistic: fisher calls $\sim$11,000 PAS per cell type
against nb\_multi's $\sim$331, a 33$\times$ size gap that pins Jaccard near zero
(median 0.008) for arithmetic reasons rather than genuine disagreement.
Containment is the statistic that answers the question; both are plotted so the
difference is visible. Three quarters of the strictest strategy's hits are not
reproduced by the more permissive one --- the opposite of what nesting would
predict if both detected the same signal, and the strongest available evidence
that at least one is dominated by false positives.

The superseded v9 numbers are withdrawn.
...
\begin{quote}
\textbf{The mean of this metric is not a measurement --- do not compare it across
conditions.} Because the proportions sum to 1.0 within each (gene, cell), the mean
over all rows is exactly $n_{\text{gene,cell}} / n_{\text{rows}}$, the reciprocal
of the mean number of PAS per gene. It is a property of the gene$\rightarrow$PAS
structure and carries no information about reads, stage or biology.

On the corrected sweep this is exact, not approximate: within every cell type the
mean proportion is \textbf{identical to six decimal places across all four
stages} (spread exactly 0.0), and across nine cell types it takes only three
distinct values (0.328633, 0.328320, 0.278112), corresponding to mean
PAS-per-gene of 3.043, 3.046 and 3.596. By comparison \texttt{classic} and
\texttt{shannon} vary across stages within a cell type by up to 0.033 and 0.014.

\textbf{The direct cause is uniform padding, and it is verified.} In a sample of
8.0M rows from the epithelial table, 7,858,254 carry \texttt{reads\_at\_pas = 0},
and \textbf{100.0000\% of them have \texttt{proportion} exactly equal to
$1/n_{\text{PAS}}$}. Uncovered (gene, cell) pairs are not omitted and not marked
missing --- they are emitted with a uniform prior, so \textbf{98.2\% of the table
is synthetic}. Only 141,745 rows (1.8\%) carry any reads.

Two column-level defects follow, and they block the obvious workarounds.
\texttt{total\_reads\_gene} is not a read count on padded rows: for 99.97\% of
zero-coverage pairs it holds the \emph{number of PAS} instead, so filtering on it
does not select covered rows --- which is why the coverage-conditioned mean for
\texttt{proportion} is identical to the unconditional one while \texttt{classic}
and \texttt{shannon} both move. And \texttt{rank} is never populated: it is 1 for
every one of $\sim$20M rows inspected, so it cannot order PAS from proximal to
distal. On the 1.8\% of rows that do carry reads the mean proportion is
\textbf{0.5627}, not 0.3286 --- the padding moves the summary by a factor of 1.7
and pins it to a constant.

This retroactively explains the previous report's own result, which recorded that
``only 47 genes showed inter-cluster proportion shift $>0.1$ (by the
mean-across-PAS metric)'' and read it as consistent with a smooth usage
distribution. A statistic that cannot vary produced almost no variation; the
near-null was structural.

\textbf{What a usable summary would require.} Keeping the PAS axis is the right
idea, but it cannot be done from this output as written: \texttt{rank} is
constant, so PAS order would have to be reconstructed from the coordinate columns,
and rows selected on \texttt{reads\_at\_pas} rather than
\texttt{total\_reads\_gene}. Until both column defects are fixed,
\texttt{proportion} should not be used for cross-condition comparison at all, by
any summary.
\end{quote}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/corrected/13_length_strategy.png}
  \caption{(a) share of each table that is zero-coverage rows; (b) mean score as
    reported versus conditioned on the gene having reads; (c) the stage
    trajectory per metric, conditioned. Aggregated over 24 cell types.}
  \label{fig:length_strategy}
\end{figure}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrrrrr}
    \toprule
    strategy & cell types & rows & mean units & zero-cov rows & mean (all) & mean ($\geq$1 read) \\
    \midrule
    \texttt{classic} & 18 & 85,404,992 & 4,426 & 96.9\% & 0.012 & 0.381 \\
    \texttt{proportion} & 18 & 463,425,077 & 7,682 & 0.0\% & 0.322 & 0.322 \\
    \texttt{shannon} & 17 & 138,621,873 & 7,704 & 95.7\% & 0.570 & 0.688 \\
    \bottomrule
  \end{tabular}
  \caption{Length metrics on the corrected cohort. The \texttt{proportion} row is
    not comparable to the other two --- see the padding note above.}
  \label{tab:length_strategy}
\end{table}

For \texttt{classic} and \texttt{shannon}, which are comparable: both tables are
$\sim$96--97\% zero-coverage rows, so the signal is carried by 3--4\% of each;
conditioning moves both means substantially (classic $0.012 \rightarrow 0.381$,
shannon $0.570 \rightarrow 0.688$); and neither shows a stage trend once
conditioned. Changing the length metric does not rescue the shortening signal,
which is the useful negative --- the Section~\ref{sec:corrected} result is a
property of the data, not of the \texttt{classic} PDUI definition.

The superseded v9 numbers are withdrawn. Note that the v9 \texttt{proportion} row
showed 47 shifting units against 4,045 for \texttt{classic} --- the constant-mean
artifact visible in the old run, and read at the time as a biological property.
\texttt{proportion per\_isoform} was not run in this sweep (memory-bound).
...
This section traces individual genes from peak detection through each
differential and length strategy. It has two halves, and the distinction
matters: Section~\ref{sec:gene_walk_corrected} shows \textbf{corrected-sweep}
per-gene usage, while the subsections after it are retained from the superseded
v9 run purely to document the \emph{output format} of each strategy panel ---
their values must not be cited.

\subsection{Corrected-sweep gene walk}
\label{sec:gene_walk_corrected}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/corrected/14_gene_walk.png}
  \caption{Per-gene PAS usage by stage for EZR, DPYD and PHACTR1 on the corrected
    sweep: read share within each stage, pooled over cell types. Full coverage
    --- 3 genes $\times$ 24 cell types $\times$ 4 stages (4,795 rows); EZR 17 PAS
    / 12,034 reads, DPYD 148 PAS / 93,988 reads, PHACTR1 22 PAS / 2,004 reads.
    Each panel draws the twelve best-supported sites in genomic order and pools
    the remainder into a labelled \texttt{other} bar. \textbf{That bar is
    load-bearing, not cosmetic: for DPYD it carries $\approx$60\% of the gene's
    reads.} Reading the top-12 alone --- or truncating the figure to them ---
    inverts the picture of where DPYD's polyadenylation actually happens.
    Replaces the two withdrawn \texttt{gene\_walk\_*} panels.}
  \label{fig:gene_walk_corrected}
\end{figure}

DPYD is the case that justifies showing the pooled bar at all. With 148 detected
PAS the top twelve account for only about 40\% of the gene's reads, so any
per-gene summary that silently caps at a top-$N$ --- as several APA tools do ---
describes a minority of the evidence while appearing to describe the gene. The
same caution applies to the proportion heatmaps in the format subsections below,
which are capped at the top 12 without such a bar.

\medskip
\noindent\textbf{The subsections that follow are format documentation only.}
They come from the superseded v9 run and predate the cell-doubling and
atlas-snap fixes. Their gene-level inferences are contradicted by the corrected
analysis --- see the superseded notes inline, and
Sections~\ref{sec:q_shortening} and~\ref{sec:q_fisher}.

\subsection{DPYD (ENSG00000188641) --- format walkthrough}
...
\begin{quote}
\textbf{Superseded.} The paragraph above is retained to document what the
walkthrough \emph{format} conveys, but its inference does not survive the
corrected analysis and must not be carried forward. ``33 of 39 tested PAS
significant'' is the pseudoreplication signature quantified in
Section~\ref{sec:q_fisher} --- Fisher calls a median 43\% of all tested PAS
significant --- and the shortening reading is subject to the detection-rate
confound of Section~\ref{sec:q_shortening}. Read this subsection as a tour of
the output panels, not as evidence about DPYD.
\end{quote}

\subsection{PHACTR1 (ENSG00000112137) --- format walkthrough}
...
\begin{quote}
\textbf{Superseded}, on the same grounds as the DPYD subsection above: ``all 19
PAS significant by Fisher'' is what a test that calls a median 43\% of PAS
significant produces on a well-covered gene (Section~\ref{sec:q_fisher}), and
per-cluster PDUI extremes are exactly where the detection-rate confound bites
hardest (Section~\ref{sec:q_shortening}). The panels illustrate the output
format; the gene-level claim is not supported by the corrected sweep.
\end{quote}
...
\begin{enumerate}
  \item \textbf{\texttt{ip\_filter\_mode} does not identify the internal-priming
    ...
    so disabling it produces byte-identical output. Comparing the two resolved
    configs directly, they differ in exactly two fields --- \texttt{ip\_filter}
    (True/False) and \texttt{genome\_fasta} (path/None) --- so \texttt{lg\_ip\_off}
    loaded no genome and ran no scan, and \emph{still} returned the same 22,633
    peaks. The nominally three-level ip axis is really two-level.
...
% -----------------------------------------------------------------------------
\section{Filter-Effect Comparison}
\label{sec:filter_effect}
% -----------------------------------------------------------------------------

\begin{quote}
\textbf{Status: the clustering result below is complete and final.} It is
computed without \texttt{fisher} or \texttt{proportion}, so it is unaffected by
the two engine fixes in flight (Section~\ref{sec:switch_atlas}). The
differential and length rows of Table~\ref{tab:filter_effect_skeleton} are
\textbf{still pending} and are marked as such.
\end{quote}

Each scenario re-runs clustering and GEX cell-typing off \emph{identical} saved
peak-calls on the 6-GSM subset, flipping exactly one PAS filter. Critically,
every scenario keeps all \textbf{400,253} PAS in its results --- the filter only
changes which PAS are used \emph{as clustering features}. So this measures the
cost of a filter, not of a smaller catalogue.
...
biology is the question that remains --- and Figure~\ref{fig:lost_distal} gives a
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/corrected/20_filter_effect_clustering.png}
  \caption{What each filter removes, and what it costs: PAS excluded from
    clustering, agreement with the baseline clustering, agreement with
    independent GEX cell types, and the cost to cells --- both those reassigned
    to a different cluster and those dropped from the analysis entirely.}
  \label{fig:filter_effect_clustering}
\end{figure}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrrrrrr}
    \toprule
    scenario & \% PAS & clusters & ARI vs & AMI vs & \% cells & \% cells & ARI vs \\
             & excl.\ & (mean)   & base   & base   & reassign & lost     & GEX \\
    \midrule
    \texttt{baseline}          &  0.0 & 14.17 & ---    & ---    & ---   & ---   & 0.423 \\
    \texttt{atlas\_filter}     &  8.7 & 13.67 & 0.834 & 0.878 &  8.44 &  0.31 & 0.423 \\
    \texttt{annot\_filter\_3utr} & \textbf{73.1} & \textbf{11.67} & \textbf{0.628} & \textbf{0.710} & \textbf{18.02} & \textbf{10.75} & \textbf{0.384} \\
    \texttt{ip\_filter}        &  8.3 & 13.50 & 0.842 & 0.878 &  6.88 &  0.31 & 0.414 \\
    \bottomrule
  \end{tabular}
  \caption{Filter effect on clustering, 6-GSM subset, mean over datasets.
    Baseline AMI vs GEX is 0.610; atlas 0.608, 3\textquotesingle{}UTR 0.577, ip
    0.609. Cell-type calls are GEX-derived and therefore independent of the PAS
    filter --- they stay at 11.3--11.5 types in every scenario, which is the
    sanity check that the pipeline did what was intended.}
  \label{tab:filter_effect_result}
\end{table}

\textbf{Headline: requiring a PAS to sit literally inside an annotated
3\textquotesingle{}UTR excludes 73\% of called PAS, and it is the only filter
that materially degrades the analysis.} Agreement with the baseline clustering
falls to ARI 0.628, agreement with independent GEX cell types falls from 0.423 to
0.384, 18\% of cells change cluster, and the partition coarsens from 14.2 to 11.7
clusters. The atlas-nonmatch and internal-priming filters remove $\approx$8\%
each and cost almost nothing: ARI 0.83--0.84 against baseline, GEX concordance
unchanged to three decimals for atlas.

\textbf{A cost that only appears when you look at cells.} The
3\textquotesingle{}UTR filter does not merely move cells between clusters --- it
\textbf{drops 2,251 of 20,933 cells (10.75\%) out of the analysis entirely},
because they fall below the minimum-PAS-per-cell threshold once 73\% of features
are gone. The other two filters lose 65 cells (0.31\%). A cell that is never
clustered contributes to no downstream result, so for many purposes this is the
more consequential number than the ARI drop, and it is invisible in any
PAS-level summary.

\subsection{Single-cell view: UMAP and cluster $\rightarrow$ cell-type flow}
\label{sec:filter_effect_viz}

The numbers above say the 3\textquotesingle{}UTR filter costs 0.039 ARI against
GEX labels. This is what that looks like at single-cell resolution, on the
shared reference dataset \texttt{GSM3516663-StageIA}, with the same conventions
as Figures~\ref{fig:umap_stages}--\ref{fig:cluster_sizes_corrected}: unlabelled
cells are drawn grey and \emph{kept}, and each panel is one named dataset
because \texttt{leiden} ids are not comparable across datasets.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/corrected/21_filter_effect_umap.png}
  \caption{Per-scenario UMAP, coloured by PAS cluster (top) and by GEX cell type
    (bottom). \textbf{Each embedding is computed independently for its scenario},
    so differences in \emph{layout} between panels are not evidence of a
    clustering difference --- UMAP coordinates are not comparable across
    separate runs. Figure~\ref{fig:filter_effect_clustering} is the quantitative
    comparison; this figure shows that every scenario yields discrete,
    cell-type-coherent structure rather than noise, and that the
    3\textquotesingle{}UTR panel does so with 90 fewer cells on this dataset.}
  \label{fig:filter_effect_umap}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/corrected/22_filter_effect_sankey.png}
  \caption{PAS cluster $\rightarrow$ GEX cell type per scenario. Ribbon colour
    is the destination cell type; pale grey ribbons are unlabelled cells, kept
    rather than dropped. The cell-type column is identical across scenarios by
    construction --- cell typing is GEX-derived and filter-independent --- so
    only the left-hand partition and the ribbon structure change.}
  \label{fig:filter_effect_sankey}
\end{figure}

\begin{quote}
\textbf{Do not read the purity numbers as a ranking.} Median cluster purity is
0.71 (baseline), 0.73 (atlas), \textbf{0.80} (3\textquotesingle{}UTR) and 0.72
(internal priming) --- the \emph{worst} scenario by every other measure scores
the \emph{highest} purity. This is the same mechanical artefact flagged for
Figure~\ref{fig:sankey_variants}: purity rises as clusters shrink and multiply
relative to the label set, so a coarser partition over the same labels can score
higher while resolving less. The 3\textquotesingle{}UTR filter drops to 13
clusters from 15 and loses 10.75\% of cells; its higher purity reflects that
loss, not better agreement. \textbf{Purity is a within-panel descriptive, never a
between-scenario score} --- ARI and AMI against the GEX labels
(Table~\ref{tab:filter_effect_result}) are the comparable statistics, and they
put the 3\textquotesingle{}UTR filter last.
\end{quote}

\subsection{Scoring the advance predictions}

Three predictions were recorded before the run. The first can now be scored.

\begin{enumerate}[nosep]
  \item \emph{"Clustering should be robust (the trim grid moved it by only
    0.24)."} \textbf{Half wrong, and the wrong half is the interesting one.}
    Robust for atlas and internal-priming; \textbf{not} robust for the
    3\textquotesingle{}UTR filter, which moved cell-type agreement by 0.039 ARI
    --- five times the entire \texttt{max\_gene\_distance} range
    (Section~\ref{sec:q_trim}) and comparable to the resolution knob. The
    intuition that "clustering is insensitive to PAS-set changes" came from
    filters that remove $\approx$8\%; it does not extend to one that removes 73\%.
  \item \emph{"Differential hit counts should fall roughly in proportion to PAS
    removed."} \textbf{Pending} the corrected differential engine.
  \item \emph{"The shortening/lengthening split should move most under
    annot-filter."} \textbf{Pending} the corrected length engine.
\end{enumerate}

\subsection{Interpretation, and the control it still needs}

Most called PAS are \emph{not} inside annotated 3\textquotesingle{}UTRs --- 73\%
sit in introns, 5\textquotesingle{} regions, extended windows or intergenic
space --- and the clustering result says a meaningful share of them carry
cell-identity signal. That is a direct argument against pipelines that restrict
APA analysis to annotated 3\textquotesingle{}UTRs by default: the restriction is
not conservative, it discards signal.

\begin{quote}
\textbf{The control that would make this airtight has not been run.} Removing
73\% of features and 10.75\% of cells will degrade any clustering somewhat, for
purely mechanical reasons. To separate \emph{"non-3\textquotesingle{}UTR PAS are
especially informative"} from \emph{"fewer features cluster worse"}, the
scenario needs a \textbf{feature-count-matched control}: drop a random 73\% of
PAS (several draws) and re-cluster. If random removal costs as much, the result
is about feature count; if it costs less, non-3\textquotesingle{}UTR PAS are
carrying disproportionate signal. This is cheap --- it is \texttt{ema reannotate}
with a shuffled mask --- and it is the single thing that would turn this section
from suggestive into conclusive. Until then, the defensible claim is the
\emph{operational} one: applying a 3\textquotesingle{}UTR filter changes the
clustering and loses a tenth of the cells, so it must not be applied silently.
\end{quote}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{lXl}
    \toprule
    \textbf{Pipeline step} & \textbf{Metric reported per scenario} & \textbf{Status} \\
    \midrule
    PAS catalogue & PAS kept, excluded, fraction retained & \textbf{done} \\
    Feature space & kept-in-results vs used-for-clustering & \textbf{done} \\
    Clustering & $n$ clusters, ARI/AMI vs baseline & \textbf{done} \\
    Cell typing & \% of cells reassigned; \% dropped entirely & \textbf{done} \\
    Differential APA & significant PAS/genes; hit-set overlap with baseline & pending engine \\
    3\textquotesingle{}UTR trend & per-cell-type slope; sign flips vs baseline & pending engine \\
    \midrule
    Feature-count-matched control & random-73\% mask, re-cluster & \textbf{not run} \\
    \bottomrule
  \end{tabularx}
  \caption{Figure slots 21--23 remain reserved for the differential, length and
    per-scenario UMAP/Sankey panels.}
  \label{tab:filter_effect_skeleton}
\end{table}
...
All figures below are in \texttt{figures/corrected/}, as both \texttt{.png}
(200\,dpi) and \texttt{.svg}. Generators, all under \texttt{reports/}: figures
01--14 from \texttt{generate\_corrected\_figures.py}, 15--19 from
\texttt{generate\_clustering\_figures.py}, and 24--36 from
\texttt{generate\_cross\_experiment\_figures.py}. All read on-disk tables from
the real \texttt{RERUN\_2026-08\_fixed} sweep --- no value is hardcoded, and a
figure whose input table is missing is skipped rather than drawn from stale
data. \textbf{Slot 23 remains reserved} for the pending filter-effect differential and
length panels (Section~\ref{sec:filter_effect}).

Schematics that remain valid from the pre-correction set
(\texttt{fig\_pipeline\_flow}, \texttt{fig\_strategy\_explained},
\texttt{fig\_math\_methods}, \texttt{fig\_tfidf\_pipeline},
\texttt{*\_explainer}) are cited in place and carry no data.

\begin{longtable}{rll}
  \toprule
  \textbf{\#} & \textbf{Description} & \textbf{File (\texttt{.png}/\texttt{.svg})} \\
  \midrule
  \endhead
  01 & Peak count, F1 across cutoffs, F1 vs peak count & \texttt{01\_strategy\_benchmark} \\
  02 & Atlas precision saturation vs recall & \texttt{02\_atlas\_saturation} \\
  03 & Count-controlled rarefaction, distance, containment & \texttt{03\_count\_controlled} \\
  04 & Internal-priming axis; trim vs PAS and cluster count & \texttt{04\_ip\_and\_trim} \\
  05 & Resolution, n\_neighbors, PAS-vs-GEX agreement & \texttt{05\_clustering} \\
  06 & Depth confound: PDUI vs zero-coverage fraction & \texttt{06\_depth\_confound} \\
  07 & Stage trajectory, reported vs coverage-conditioned & \texttt{07\_stage\_trajectory} \\
  08 & Fisher significance-rate distribution & \texttt{08\_fisher\_power} \\
  09 & Recurrence and effect size, recurrent vs private & \texttt{09\_recurrence} \\
  10 & Lost distal elements vs length-matched control & \texttt{10\_lost\_distal\_elements} \\
  11 & Cancer-gene and Hallmark enrichment, crude vs adjusted & \texttt{11\_gene\_sets} \\
  12 & Differential strategy comparison (fisher vs nb\_multi) & \texttt{12\_diff\_strategy} \\
  13 & Length-metric comparison (classic / shannon / proportion) & \texttt{13\_length\_strategy} \\
  14 & Corrected-sweep gene walk (EZR, DPYD, PHACTR1) by stage & \texttt{14\_gene\_walk} \\
  15 & Cohort UMAP by stage, PAS cluster and cell type & \texttt{15\_umap\_cohort\_stages} \\
  16 & UMAP across the six clustering variants & \texttt{16\_umap\_clustering\_grid} \\
  17 & PAS cluster $\rightarrow$ cell-type Sankey, per variant & \texttt{17\_sankey\_clustering\_grid} \\
  18 & PAS cluster $\rightarrow$ cell-type Sankey, per strategy & \texttt{18\_sankey\_peak\_strategies} \\
  19 & Cluster count and size distribution across experiments & \texttt{19\_cluster\_sizes} \\
  20 & Filter effect on clustering, cells and GEX concordance & \texttt{20\_filter\_effect\_clustering} \\
  21 & Per-scenario UMAP by cluster and by cell type & \texttt{21\_filter\_effect\_umap} \\
  22 & Per-scenario cluster $\rightarrow$ cell-type Sankey & \texttt{22\_filter\_effect\_sankey} \\
  24 & Atlas precision against shifted and uniform nulls & \texttt{24\_atlas\_null\_control} \\
  25 & Excess precision over null, per arm and cutoff & \texttt{25\_atlas\_excess\_over\_null} \\
  26 & PAS-set Jaccard between peak strategies & \texttt{26\_pas\_set\_jaccard} \\
  27 & PAS yield against cell-type ARI & \texttt{27\_yield\_vs\_celltype\_ari} \\
  28 & Trim window vs ARI, cluster count and yield & \texttt{28\_trim\_sensitivity} \\
  29 & \% of cells whose cell-type call each parameter changes & \texttt{29\_celltype\_reassignment} \\
  30 & Reannotate reproducibility: identical params, 3 md5 groups & \texttt{30\_reannotate\_reproducibility} \\
  31 & Every swept knob ranked by effect on the biology & \texttt{31\_knob\_ranking} \\
  32 & Resolution: ARI peaks at 1.0, silhouette does not & \texttt{32\_resolution\_ari\_silhouette} \\
  33 & Mean PDUI by cell type $\times$ stage & \texttt{33\_pdui\_celltype\_stage} \\
  34 & Trend slope with vs without the depth control & \texttt{34\_trend\_depth\_confound} \\
  35 & Fisher hits vs cell count, sized by effect & \texttt{35\_fisher\_pseudoreplication} \\
  36 & Recurrence distribution and top recurrent genes & \texttt{36\_recurrent\_switches} \\
  37 & Both differential strategies per cell type & \texttt{37\_diff\_strategies\_by\_celltype} \\
  38 & Per-cell-type differential APA volcanoes & \texttt{38\_volcano\_by\_celltype} \\
  39 & Both length metrics (classic, shannon) by stage & \texttt{39\_length\_metrics\_by\_stage} \\
  40 & Most recurrent large-effect switch genes, named & \texttt{40\_top\_switch\_genes} \\
  41 & Hallmark enrichment of switch-gene sets & \texttt{41\_hallmark\_switch\_enrichment} \\
  \bottomrule
\end{longtable}
