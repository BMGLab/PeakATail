...
| C15 | **UMAP across stages** (cluster + cell type) | `15_umap_cohort_stages.png` | `extra/umap_cells.tsv.gz` |
| C16 | UMAP, six clustering variants | `16_umap_clustering_grid.png` | `extra/umap_cells.tsv.gz` |
| C17 | **Sankey** PAS cluster → cell type, per clustering variant | `17_sankey_clustering_grid.png` | `extra/cluster_celltype_counts.tsv` |
| C18 | Sankey, per peak-calling strategy | `18_sankey_peak_strategies.png` | `extra/cluster_celltype_counts.tsv` |
| C19 | Cluster sizes across all 19 experiments | `19_cluster_sizes.png` | `extra/cluster_celltype_counts.tsv` |

Figures C15–C19 are generated by `reports/generate_clustering_figures.py` from real
`clusters.h5ad` / `*_pas_labeled.h5ad` artifacts (exported by
`_server_scripts/s6_clustering_viz_data.py`).
...
| 12 | 6-panel clustering comparison | `fig_main_comparison.png` | **DATA — replaced by C15** |
| 13 | Sankey flow diagram | `fig_sankey.png` | **DATA — replaced by C17** |
| 14 | Cluster sizes | `fig_cluster_sizes.png` | **DATA — replaced by C19** |
| 15 | APA subclusters | `fig_subcluster.png` | **DATA — replaced by C16** |
