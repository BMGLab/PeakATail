...
def harvest_diff(run: Path, out: Path, sym: pd.DataFrame, rng_seed: int = 0) -> None:
    rng = np.random.default_rng(rng_seed)
    strat_rows, volcano, top_rows = [], [], []

    for ct in celltype_dirs(run):
        name = ct.name
        nb_path = ct / "nb_multi" / "differential" / "nb_multi_omnibus.tsv"
        nb_sig: set[int] = set()
        nb_tests = nb_sig_n = 0
        if nb_path.exists():
        ...
        for tsv in sorted((ct / "fisher" / "differential").glob("fisher_*.tsv")):
            ...
            strat_rows.append({
                ...
                "n_tests_fisher": int(len(f)),
                "n_sig_fisher": int(sig.sum()),
                "frac_sig_fisher": round(float(sig.mean()), 4),
                "n_sig_fisher_large": int(big.sum()),
                "frac_sig_fisher_large": round(float(big.mean()), 4),
                "n_tests_nb_multi": int(nb_tests),
                "n_sig_nb_multi": int(nb_sig_n),
                "frac_sig_nb_multi": round(nb_sig_n / nb_tests, 4) if nb_tests else np.nan,
                "n_sig_both": int(inter),
                "jaccard_sig": round(inter / union, 4) if union else np.nan,
                "frac_fisher_sig_also_nb": round(inter / len(f_sig), 4) if f_sig else np.nan,
                "n_cells": int(
...
def harvest_enrichment(run: Path, out: Path, hallmark: Path, sym: pd.DataFrame) -> None:
...
def main() -> int:
    ...
    harvest_diff(a.run, a.out, sym)
    if not a.skip_shannon:
        harvest_shannon(a.run, a.out)
    if a.hallmark:
        harvest_enrichment(a.run, a.out, a.hallmark, sym)
    return 0
